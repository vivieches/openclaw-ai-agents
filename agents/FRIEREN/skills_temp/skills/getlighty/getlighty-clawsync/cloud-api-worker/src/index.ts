  ...
  if (c.req.method === "OPTIONS") {
    return new Response(null, {
      ...
      headers: {
        ...
        "Access-Control-Allow-Headers": "Content-Type, Authorization, X-ClawSync-Signature, X-ClawSync-Hash, X-ClawSync-Timestamp",
...
async function auth(
  ...
  return authenticateVault(db, vaultId, req.header("X-ClawSync-Signature"), sigPayload);
}
...
app.put("/v1/vaults/:id/sync", async (c) => {
  ...
  const archiveHash = c.req.header("X-ClawSync-Hash") || "";
...
app.get("/v1/vaults/:id/sync", async (c) => {
  const vaultId = c.req.param("id");
  const ts = c.req.header("X-ClawSync-Timestamp") || "";
  ...
  return new Response(obj.body, { headers: { "Content-Type": "application/gzip", "X-ClawSync-Hash": latest.hash_sha256, "X-ClawSync-Version": latest.id } });
});
...
app.get("/v1/vaults/:id/profiles/:profile/sync", async (c) => {
  ...
  const obj = await c.env.STORAGE.get(latest.s3_key);
  if (!obj) return c.json({ error: "Archive not found" }, 404);
  return new Response(obj.body, { headers: { "Content-Type": "application/gzip", "X-ClawSync-Hash": latest.hash_sha256, "X-ClawSync-Version": latest.id, "X-ClawSync-Profile": profileName } });
});
