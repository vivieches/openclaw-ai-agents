...
PRIVATE_KEY="$KEY_DIR/clawsync_ed25519"
PUBLIC_KEY="$KEY_DIR/clawsync_ed25519.pub"
SIGNING_KEY="$KEY_DIR/clawsync_ed25519.pem"

timestamp() { date -u +"%Y-%m-%dT%H:%M:%SZ"; }
log() { echo "[clawsync:keys $(timestamp)] $*"; }
...
cmd_generate() {
  ...
  local comment
  comment="clawsync@$(hostname -s 2>/dev/null || echo unknown)-$(date +%s)"
...
cmd_rotate() {
  ...
  local ts
  ts=$(date +%Y%m%d-%H%M%S)
  mv "$PRIVATE_KEY" "$archive_dir/clawsync_ed25519.$ts"
  mv "$PUBLIC_KEY" "$archive_dir/clawsync_ed25519.$ts.pub"
  [[ -f "$SIGNING_KEY" ]] && mv "$SIGNING_KEY" "$archive_dir/clawsync_ed25519.$ts.pem"
  ...
  log "  For ClawSync Cloud: clawsync.sh cloud update-key"
  log "  For Git: add the new public key to your repo's deploy keys"
  log "  For others: re-run 'clawsync.sh provider setup <name>'"
}
...
cmd_verify() {
  ...
  local test_file
  test_file=$(mktemp)
  echo "clawsync-verify-test" > "$test_file"

  if ssh-keygen -Y sign -f "$PRIVATE_KEY" -n clawsync "$test_file" &>/dev/null; then
