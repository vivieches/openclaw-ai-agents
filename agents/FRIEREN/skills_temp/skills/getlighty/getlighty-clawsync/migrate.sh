...
log() { echo "[clawsync:migrate $(timestamp)] $*"; }
...
cmd_pull() {
  ...
  if [[ -z "$provider" ]]; then
    log "No provider configured. Run 'clawsync.sh provider setup <name>' first."
    return 1
  fi

  echo ""
  echo "ClawSync Migration — Pull"
  echo "=========================="
  echo ""
  echo "This will:"
  ...
  CLAWSYNC_PROFILE="$chosen" bash "$SCRIPT_DIR/src/sync-engine.sh" pull
