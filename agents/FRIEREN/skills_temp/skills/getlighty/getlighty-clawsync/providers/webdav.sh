#!/usr/bin/env bash
# ClawSync Provider — WebDAV (Nextcloud, ownCloud, etc., via rclone)
set -euo pipefail
VAULT_DIR="$HOME/.clawsync"; CONFIG="$VAULT_DIR/config.yaml"; PROVIDER_CONFIG="$VAULT_DIR/.provider-webdav.json"
RCLONE_REMOTE="clawsync-webdav"; REMOTE_DIR="ClawSync"
timestamp() { date -u +"%Y-%m-%dT%H:%M:%SZ"; }; log() { echo "[clawsync:webdav $(timestamp)] $*"; }
