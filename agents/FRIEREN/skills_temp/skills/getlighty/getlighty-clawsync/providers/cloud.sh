#!/usr/bin/env bash
# ClawSync Cloud Provider — Managed storage with pay-per-MB pricing
...
API_BASE="https://clawsync-api.ovisoftblue.workers.dev/v1"

timestamp() { date -u +"%Y-%m-%dT%H:%M:%SZ"; }
log() { echo "[clawsync:cloud $(timestamp)] $*"; }

# Use --http1.1 to avoid HTTP/2 protocol errors on some systems
CURL="curl --http1.1"

get_profile_name() {
  if [[ -n "${CLAWSYNC_PROFILE:-}" ]]; then echo "$CLAWSYNC_PROFILE"; return; fi
  local name
...
get_public_key() {
  cat "$KEY_DIR/clawsync_ed25519.pub" 2>/dev/null
}

get_private_key() {
  echo "$KEY_DIR/clawsync_ed25519"
}
...
cmd_setup() {
  ...
  log "Registering with ClawSync Cloud..."
    ...
    cat > "$CLOUD_CONFIG" <<JSON
{
  "provider": "cloud",
  "email": "$email",
  "public_key_fingerprint": "$(ssh-keygen -lf "$KEY_DIR/clawsync_ed25519.pub" 2>/dev/null | awk '{print $2}')",
  "hostname": "$hostname_str",
...
JSON
    echo ""
    log "✓ Cloud config saved. You'll be notified when ClawSync Cloud launches."
...
JSON
  ...
  log "✓ Registered with ClawSync Cloud"
...
cmd_push() {
  ...
  openssl enc -aes-256-cbc -pbkdf2 -salt \
    ...
    -pass "file:$KEY_DIR/clawsync_ed25519" 2>/dev/null
  ...
  log "Pushing to ClawSync Cloud ($size_mb MB, profile: $profile_name, encrypted)..."
  ...
  http_code=$($CURL -sf -o /dev/null -w "%{http_code}" \
    ...
    -H "X-ClawSync-Signature: $signature" \
    -H "X-ClawSync-Hash: $archive_hash" \
...
cmd_pull() {
  ...
  local profile_name
  profile_name=$(get_profile_name)
  log "Pulling from ClawSync Cloud (profile: $profile_name)..."

  $CURL -sf -o "$archive" \
    -H "X-ClawSync-Signature: $signature" \
    -H "X-ClawSync-Timestamp: $ts_now" \
...
cmd_test() {
  ...
  log "Testing ClawSync Cloud connection..."
...
print(f'  Instances:     {d.get(\"instance_count\", \"?\")}')" 2>/dev/null
    ...
    echo ""
    echo "☁️  ClawSync Cloud — Estimated Usage"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
