#!/usr/bin/env bash
# ClawSync Provider — Dropbox (via rclone)
set -euo pipefail
VAULT_DIR="$HOME/.clawsync"; CONFIG="$VAULT_DIR/config.yaml"; PROVIDER_CONFIG="$VAULT_DIR/.provider-dropbox.json"
RCLONE_REMOTE="clawsync-dropbox"; REMOTE_DIR="ClawSync"
timestamp() { date -u +"%Y-%m-%dT%H:%M:%SZ"; }; log() { echo "[clawsync:dropbox $(timestamp)] $*"; }
