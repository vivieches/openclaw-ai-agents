#!/usr/bin/env bash
# ClawSync Provider — FTP / SFTP
set -euo pipefail
VAULT_DIR="$HOME/.clawsync"; CONFIG="$VAULT_DIR/config.yaml"; PROVIDER_CONFIG="$VAULT_DIR/.provider-ftp.json"
timestamp() { date -u +"%Y-%m-%dT%H:%M:%SZ"; }; log() { echo "[clawsync:ftp $(timestamp)] $*"; }
...
cmd_setup() {
  ...
  read -rp "Username: " user; read -rp "Remote path [/clawsync]: " rpath; rpath="${rpath:-/clawsync}"
  echo ""
  echo "Authentication: your ClawSync Ed25519 key will be used."
...
cmd_push() {
  _load; local key="$VAULT_DIR/keys/clawsync_ed25519"
