#!/usr/bin/env bash
# ClawSync Provider — Git (auto-commit + push with Ed25519 key)
set -euo pipefail
VAULT_DIR="$HOME/.clawsync"; CONFIG="$VAULT_DIR/config.yaml"; REPO_DIR="$VAULT_DIR/.git-local"
PROVIDER_CONFIG="$VAULT_DIR/.provider-git.json"
timestamp() { date -u +"%Y-%m-%dT%H:%M:%SZ"; }; log() { echo "[clawsync:git $(timestamp)] $*"; }

_git() { GIT_SSH_COMMAND="ssh -i $VAULT_DIR/keys/clawsync_ed25519 -o StrictHostKeyChecking=no" git -C "$REPO_DIR" "$@"; }
...
cmd_setup() {
  ...
  echo "────────────────────────────────"
  cat "$VAULT_DIR/keys/clawsync_ed25519.pub" 2>/dev/null
  echo "────────────────────────────────"
  ...
  if [[ -d "$REPO_DIR/.git" ]]; then
    ...
    GIT_SSH_COMMAND="ssh -i $VAULT_DIR/keys/clawsync_ed25519 -o StrictHostKeyChecking=no" git clone "$remote_url" "$REPO_DIR" 2>/dev/null || {
