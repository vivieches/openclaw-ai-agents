...
# Usage: clawsync.sh <command> [args]
...
cmd_init() {
  if [[ -f "$CONFIG" ]]; then
    ...
    log "Run 'clawsync.sh status' or 'clawsync.sh provider setup <name>'"
    return 0
  fi

  log "Initializing ClawSync v$VERSION..."
  ...
  # Create config
  cat > "$CONFIG" <<YAML
# ClawSync Configuration v$VERSION
# Generated: $(timestamp)
...
# Set by 'clawsync.sh provider setup <name>'
provider: ""
...
YAML
  ...
  echo "  clawsync.sh provider setup cloud      ← managed, easiest"
  echo "  clawsync.sh provider setup gdrive     ← Google Drive"
  echo "  clawsync.sh provider setup dropbox    ← Dropbox"
  echo "  clawsync.sh provider setup git        ← any Git repo"
  echo "  clawsync.sh provider setup ftp        ← FTP/SFTP server"
  echo "  clawsync.sh provider setup s3         ← S3 bucket"
  echo "  clawsync.sh provider setup local      ← USB/NAS"
  echo ""
}
...
cmd_status() {
  if [[ ! -f "$CONFIG" ]]; then
    echo "ClawSync not initialized. Run: clawsync.sh init"
    return 1
  fi
...
cmd_profile() {
  ...
  case "$subcmd" in
    ...
    rename)
      local new_name="${2:-}"
      if [[ -z "$new_name" ]]; then
        echo "Usage: clawsync.sh profile rename <name>"
        return 1
      fi
    ...
    list)
      ...
      CLAWSYNC_PROFILE="$(get_profile_name)" bash "$PROVIDERS_DIR/${provider}.sh" list-profiles
      ;;
    pull)
      local target="${2:-}"
      if [[ -z "$target" ]]; then
        echo "Usage: clawsync.sh profile pull <name>"
        echo "Use 'clawsync.sh profile list' to see available profiles."
        return 1
      fi
      log "Pulling profile: $target"
      CLAWSYNC_PROFILE="$target" bash "$SRC_DIR/sync-engine.sh" pull
      log "Restored profile: $target"
      ;;
    *)
      echo "Usage: clawsync.sh profile {show|rename|list|pull}"
      ;;
  esac
}
...
cmd_help() {
  echo ""
  echo "🦞 ClawSync v$VERSION — Portable Agent Environment"
  echo ""
  echo "Setup:"
  ...
  echo "  cloud signup            Quick setup for ClawSync Cloud"
  echo ""
  echo "Sync:"
...
case "${1:-help}" in
  ...
  sync)
    case "${2:-status}" in
      ...
      *)      echo "Usage: clawsync.sh sync {start|stop|push|pull|status}" ;;
    esac ;;
