...
async function authenticateVault(
  ...
  if (!signature) {
    return { ok: false, error: "Missing X-ClawSync-Signature header" };
  }
...
app.put("/v1/vaults/:id/sync", async (c) => {
  ...
  const signature = c.req.header("X-ClawSync-Signature");
...
app.get("/v1/vaults/:id/sync", async (c) => {
  const vaultId = c.req.param("id");
  const signature = c.req.header("X-ClawSync-Signature");
  const timestamp = c.req.header("X-ClawSync-Timestamp") || "";
  ...
  return new Response(Buffer.from(data), {
    headers: {
      ...
      "X-ClawSync-Hash": latest.hash_sha256,
      "X-ClawSync-Version": latest.id,
    },
  });
});
...
await db.migrate();
console.log(`ClawSync Cloud API listening on :${port}`);
