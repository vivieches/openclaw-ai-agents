...
Uploads a PDF + password to Solutions API password-protect endpoint, polls until done,
then prints JSON with the download URL for the protected PDF.
...
Usage:
  python scripts/password_protect_pdf.py --pdf "/path/to/file.pdf" --password "secret" --api-key "..."
Or:
  export SOLUTIONS_API_KEY="..."
  python scripts/password_protect_pdf.py --pdf "/path/to/file.pdf" --password "secret"
"""
    ...
    while True:
        ...
        if status in {"failed", "error"}:
            return {
                ...
                "error": "job_failed",
                "message": "Password-protect job failed.",
                "raw": info,
            }
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Password-protect a PDF via Solutions API.")
    ap.add_argument("--pdf", required=True, help="Path to input PDF")
    ap.add_argument("--password", required=True, help="Password for PDF (will not be echoed)")
