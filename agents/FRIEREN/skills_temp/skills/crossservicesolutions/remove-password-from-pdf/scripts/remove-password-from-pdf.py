...
Uploads a password-protected PDF + its current password to Solutions API remove-password endpoint,
polls until done, then prints JSON with the download URL for the unlocked PDF.
...
Usage:
  python scripts/remove_password_from_pdf.py --pdf "/path/to/protected.pdf" --password "secret" --api-key "..."
Or:
  export SOLUTIONS_API_KEY="..."
  python scripts/remove_password_from_pdf.py --pdf "/path/to/protected.pdf" --password "secret"
"""
...
def create_job(
    ...
    if resp.status_code >= 400:
        # Do not echo the password. Just return server-provided message.
    ...
    while True:
        ...
        if status in {"failed", "error"}:
            return {
                ...
                "error": "job_failed",
                "message": "Remove-password job failed.",
                "raw": info,
            }
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Remove password from a PDF via Solutions API.")
    ap.add_argument("--pdf", required=True, help="Path to input PDF")
    ap.add_argument("--password", required=True, help="Current password (will not be echoed)")
