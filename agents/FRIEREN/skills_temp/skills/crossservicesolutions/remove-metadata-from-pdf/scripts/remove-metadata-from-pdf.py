...
Uploads one or multiple PDFs to Solutions API remove-metadata endpoint, polls until done,
then prints JSON with download URL(s) for the cleaned PDFs and/or ZIP.
...
Usage:
  python scripts/remove_metadata_from_pdf.py --pdf "/path/to/a.pdf" --pdf "/path/to/b.pdf" --api-key "..."
Or:
  export SOLUTIONS_API_KEY="..."
  python scripts/remove_metadata_from_pdf.py --pdf "/path/to/a.pdf"
"""
...
def create_job(
    ...
    # multipart/form-data with multiple PDFs under the SAME key: "files"
    ...
    while True:
        ...
        if status in {"failed", "error"}:
            return {
                ...
                "error": "job_failed",
                "message": "Remove-metadata job failed.",
                "raw": info,
            }
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Remove metadata from PDFs via Solutions API.")
        ...
        print(json.dumps(
            {
                "status": "error",
                "error": "missing_pdfs",
                "message": "Please provide at least one PDF (repeat --pdf).",
            },
            ensure_ascii=False,
            indent=2
        ))
