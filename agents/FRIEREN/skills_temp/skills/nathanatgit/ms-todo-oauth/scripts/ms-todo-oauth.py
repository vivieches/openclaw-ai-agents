...
class MicrosoftTodoClient:
    ...
    DEFAULT_CLIENT_ID = "ca6ec244-002c-435b-bafd-06e470d37edc"

    def __init__(self, client_id: Optional[str] = None, client_secret: Optional[str] = None, tenant_id: str = "common", redirect_uri: str = "http://localhost:8000/callback", cache_file: Optional[str] = None, debug: bool = False):
        """
        Initialize the client

        Args:
            client_id: Azure application client ID (optional, uses built-in ID by default)
            client_secret: Client secret (required for confidential client flow)
            tenant_id: Tenant ID, default is "common"
            redirect_uri: OAuth2 redirect URI (default: http://localhost:8000/callback)
        ...
        self.tenant_id = tenant_id  # Use the provided tenant_id (consumers, common, or specific tenant)
    ...
    def get_authorization_url(self) -> str:
        """
        Get OAuth2 authorization URL (Step 1: Generate login link)
    ...
    def acquire_token_by_auth_code(self, auth_code: str) -> bool:
        """
        Exchange authorization code for access token (Step 2: Complete login)

        Args:
            auth_code: Authorization code from redirect URI

        Returns:
            True if token acquisition is successful
        """
        app = msal.ConfidentialClientApplication(
            self.client_id,
            client_credential=self.client_secret,
        ...
        try:
            ...
            if "access_token" in result:
                ...
                error_desc = result.get("error_description", "No details")
    ...
    def get_device_code_flow(self) -> Optional[Dict[str, Any]]:
        """
        Get device code flow information (DEPRECATED - Use OAuth2 instead)
        
        This method is kept for backward compatibility but OAuth2 is recommended.
        """
        print("⚠️  Device code flow is deprecated. Use OAuth2 instead:")
        print("  ms-todo-oauth.py login get")
        return None

    def verify_device_code_flow(self) -> bool:
        """
        Verify device code flow (DEPRECATED - Use OAuth2 instead)
        
        This method is kept for backward compatibility but OAuth2 is recommended.
        """
        print("⚠️  Device code flow is deprecated. Use OAuth2 instead:")
        print("  ms-todo-oauth.py login verify")
        return False
...
# ==================== Task List Management ====================
    ...
    def create_task(
        ...
        Args:
            ...
            importance: Priority level - "low", "normal", or "high" (default: "normal")
            categories: List of category/tag strings (optional)
            recurrence: Recurrence pattern dict from Graph API format (optional)
        ...
        return self._make_request(f"/me/todo/lists/{list_id}/tasks", method="POST", data=task_data)
    ...
    def complete_task(self, list_id: str, task_id: str) -> Dict[str, Any]:
        ...
        return self._make_request(f"/me/todo/lists/{list_id}/tasks/{task_id}", method="PATCH", data=task_data)

    def update_task(self, list_id: str, task_id: str, task_data: Dict) -> Dict[str, Any]:
        """Update a task"""
        return self._make_request(f"/me/todo/lists/{list_id}/tasks/{task_id}", method="PATCH", data=task_data)
...
def _parse_ms_datetime(date_string: str) -> datetime:
    """
    Parse Microsoft Graph API datetime strings
    
    Microsoft returns dates like: 2026-02-20T00:00:00.0000000
    Python's fromisoformat() only handles up to 6 decimal places
    
    Args:
        date_string: Datetime string from Microsoft Graph API
    ...
    # Handle 7 decimal places (Microsoft format) by truncating to 6
    # Pattern: YYYY-MM-DDTHH:MM:SS.0000000
    if '.' in date_string:
        ...
        if tz_sep_pos > 0:
            frac = remaining[:tz_sep_pos]
            tz = remaining[tz_sep_pos:]
            if len(frac) > 6:
                frac = frac[:6]  # Truncate to 6 decimal places
            date_string = f"{base}.{frac}{tz}"
        else:
            # No timezone
            frac = remaining
            if len(frac) > 6:
                frac = frac[:6]  # Truncate to 6 decimal places
            date_string = f"{base}.{frac}"
...
def cmd_today(args, client):
    ...
    for list_name, tasks in all_tasks.items():
        for task in tasks:
            ...
            if due_date:
                task_date = _parse_ms_datetime(due_date).date()
                if task_date == today:
...
def cmd_overdue(args, client):
    ...
    for list_name, tasks in all_tasks.items():
        for task in tasks:
            ...
            due_date = task.get("dueDateTime", {}).get("dateTime")
            if due_date:
                task_date = _parse_ms_datetime(due_date)
                if task_date < now:
...
def cmd_login_get(args, client):
    ...
    print(f"  {auth_url}\n")
    print("After authorization, you will be redirected to a callback URL.")
    print("Copy the 'code' parameter from the callback URL and run:\n")
    print("  ms-todo-oauth.py login verify <authorization_code>\n")
    print("=" * 70 + "\n")
...
def cmd_login_verify(args, client):
    ...
    if not args.code:
        ...
        print("Usage: ms-todo-oauth.py login verify <authorization_code>")
        sys.exit(1)
    
    if client.acquire_token_by_auth_code(args.code):
        print("\n✓ Authentication successful!")
        print("✓ Login information saved, you will be logged in automatically next time.")
    else:
...
def create_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="ms-todo-oauth.py",
        description="Microsoft To Do command line tool",
        epilog='Example: ms-todo-oauth.py add "Complete report" -l work -p high -d 3',
    )

    parser.add_argument("-v", "--verbose", action="store_true", help="Show detailed information")
    parser.add_argument("--debug", action="store_true", help="Enable debug mode (show API requests and responses)")
    parser.add_argument("--reauth", action="store_true", help="Force re-authentication by clearing the token cache")
    ...
    login_subparsers.add_parser("get", help="Get OAuth2 authorization URL")
    verify_parser = login_subparsers.add_parser("verify", help="Verify authorization code and complete login")
    verify_parser.add_argument("code", help="Authorization code from redirect URI")
...
def main():
    ...
    if args.reauth:
        print("Force re-authentication: Clearing token cache...")
        client.logout()
