...
def validate_skill_name(skill_name: str):
    """Validate skill name against basic spec rules."""
    ...
    if len(name) > 64:
        return False, f"Skill name is too long ({len(name)} characters). Maximum is 64 characters."
    return True, ""
...
def init_skill(skill_name, path):
    ...
    try:
        # Ensure consistent UTF-8 encoding across platforms (Windows default may be non-UTF-8).
    ...
    try:
        ...
        try:
            ...
            # Best-effort on Windows; permissions may not map 1:1.
            pass
...
def main():
    ...
    ok, error = validate_skill_name(skill_name)
    if not ok:
        print(f"❌ Error: Invalid skill name '{skill_name}': {error}")
        sys.exit(1)
