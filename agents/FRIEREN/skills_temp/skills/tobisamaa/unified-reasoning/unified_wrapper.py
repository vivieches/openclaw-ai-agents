...
class Unified_ReasoningSkill(UnifiedSkill):
    def __init__(self):
        super().__init__("unified-reasoning")
    ...
    def _load_original(self):
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location(
                "unified_reasoning",
                Path(__file__).parent / "unified_reasoning.py"
            )
    ...
    def can_handle(self, goal: str) -> float:
        ...
        if any(word in goal_lower for word in ["reason", "think", "analyze", "decide", "plan"]):
            return 0.90
