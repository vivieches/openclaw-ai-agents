...
class Skill_OrchestraSkill(UnifiedSkill):
    def __init__(self):
        super().__init__("skill-orchestra")
    ...
    def _load_original(self):
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location(
                "skill_orchestra",
                Path(__file__).parent / "skill_orchestra.py"
            )
    ...
    def can_handle(self, goal: str) -> float:
        ...
        if any(word in goal_lower for word in ["route", "skill", "capability", "which skill"]):
            return 0.85
