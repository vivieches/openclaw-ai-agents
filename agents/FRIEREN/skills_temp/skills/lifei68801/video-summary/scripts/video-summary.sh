...
call_llm() {
    ...
    if [[ -z "$OPENAI_API_KEY" ]]; then
        # Return structured output for agent to process | 返回结构化输出供 agent 处理
...
EOF
    ...
    # Use OpenAI API directly | 直接调用 OpenAI API
    echo -e "${BLUE}正在调用 LLM 生成摘要...${NC}" >&2
...
请用中文输出。"
    ...
    if [[ $? -ne 0 ]]; then
        echo -e "${RED}Error: Failed to call LLM API${NC}" >&2
        return 1
    fi
    
    local summary=$(echo "$response" | jq -r '.choices[0].message.content // empty')
    
    if [[ -z "$summary" ]]; then
        echo -e "${RED}Error: Empty response from LLM${NC}" >&2
        echo "$response" | jq . >&2
        return 1
    fi
