...
**纯 Node.js，零外部依赖即可运行。Windows / macOS / Linux 通用。可选 SQLite 自动升级。**
...
- 🆕 **新增智能后端**：检测到 better-sqlite3 自动升级 SQLite + FTS5，否则降级 JSON
...
| **智能后端** | 有 better-sqlite3 → SQLite + FTS5；没有 → JSON 降级（零依赖也能用） |
...
| 特性 | 说明 |
|------|------|
| **FTS5 全文搜索** | 比 JSON 关键词匹配精准得多，支持中文分词 |
...
1. 安装 better-sqlite3
2. 运行任何命令
3. 自动检测 → 自动创建 memory.db → 自动导入 JSON 数据
4. 完成，显示 `[sqlite]`

**降级也无感：** 卸载 better-sqlite3 后自动回到 JSON 模式（JSON 文件一直保持更新）。
...
| 要求 | 说明 |
|------|------|
| **Node.js** | 18+（安装 OpenClaw 时已有） |
| **磁盘** | 通常 < 1MB（SQLite 模式 < 5MB） |
| **better-sqlite3** | **可选**，`npm install better-sqlite3` 自动升级 |
...
### Q: stats 显示 [json] 但我装了 better-sqlite3？

检查 better-sqlite3 是否在 Node.js 能找到的路径上（全局安装或技能目录下安装）。运行 `node -e "require('better-sqlite3')"` 验证。
