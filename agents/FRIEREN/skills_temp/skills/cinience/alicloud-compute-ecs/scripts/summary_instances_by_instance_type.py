#!/usr/bin/env python3
"""Summarize ECS instance counts by instance type across all regions."""
...
def main() -> int:
    ...
    for region_id in list_regions():
        ...
        for inst in iter_instances(client, region_id):
            counter[inst.instance_type] += 1

    lines = ["instance_type\tcount"]
    for itype, cnt in sorted(counter.items()):
