#!/usr/bin/env python3
"""Summarize ECS instance counts by VPC across all regions."""
...
def main() -> int:
    ...
    for region_id in list_regions():
        ...
        for inst in iter_instances(client, region_id):
            vpc_id = None
            if inst.vpc_attributes:
                vpc_id = inst.vpc_attributes.vpc_id
            key = vpc_id or "(classic)"
            counter[key] += 1

    lines = ["vpc_id\tcount"]
    for vpc_id, cnt in sorted(counter.items()):
        lines.append(f"{vpc_id}\t{cnt}")
