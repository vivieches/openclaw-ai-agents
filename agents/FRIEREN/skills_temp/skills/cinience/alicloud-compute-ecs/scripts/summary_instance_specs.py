#!/usr/bin/env python3
"""Summarize ECS instance specs across all regions."""
...
def main() -> int:
    ...
    for region_id in list_regions():
        ...
        for inst in iter_instances(client, region_id):
            spec = f"{inst.instance_type} | {inst.cpu} vCPU | {inst.memory} GiB"
            counter[spec] += 1

    lines = []
    for spec, cnt in counter.most_common():
        lines.append(f"{cnt}\t{spec}")
