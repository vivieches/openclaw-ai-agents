...
def _read_text(value: Optional[str], file_path: Optional[str], field_name: str) -> str:
    if value is not None and file_path is not None:
        raise SystemExit(f"{field_name}: 不能同时指定文本和文件")
    if value is None and file_path is None:
        raise SystemExit(f"{field_name}: 需要通过 --{field_name} 或 --{field_name}-file 指定内容")
...
def main(argv: Optional[List[str]] = None) -> None:
    parser = argparse.ArgumentParser(
        description="根据原始文本与匿名化 hide 文本生成 pair 映射",
        formatter_class=argparse.RawTextHelpFormatter,
        epilog=(
            "示例:\n"
            "  python pair_core.py --input '原句' --hide '<实体[001].Seg1.Seg2>'\n"
            "  python pair_core.py --input-file input.txt --hide-file hide.txt --dump-self-check\n"
        ),
    )
    parser.add_argument('--input', help='原始 input_text 文本内容')
    parser.add_argument('--input-file', help='从文件读取原始 input_text')
    parser.add_argument('--hide', help='包含标签的 hide 文本内容')
    parser.add_argument('--hide-file', help='从文件读取 hide 文本')
    parser.add_argument('--include-quote-delims', action='store_true', help='与批处理一致，默认关闭书名号定界符的原子化')
    parser.add_argument('--dump-self-check', action='store_true', help='输出自检详情到 stderr')
