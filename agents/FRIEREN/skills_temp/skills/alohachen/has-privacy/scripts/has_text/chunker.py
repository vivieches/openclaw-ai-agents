...
def chunk_text(
    ...
    while remaining:
        ...
        # Use ratio: chars_per_token ≈ len(remaining) / remaining_tokens
        chars_per_token = len(remaining) / remaining_tokens
        estimated_chars = int(max_tokens * chars_per_token)
        # Add a small buffer to search around the boundary
        search_end = min(estimated_chars + 50, len(remaining))
        search_text = remaining[:search_end]
        ...
        chunk_text_str = remaining[:split_pos]
