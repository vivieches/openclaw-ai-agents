...
#################################订单管理对象###########################
"""
提供单笔普通订单的订单管理对象，支持数量、价格检查、大单拆分、定时撤单、资金/持仓检查、涨停不卖、跌停不买、买卖涨跌幅限制等，具体的使用示例参考内置策略中的“日频报单场景.py”
注意事项：
...
支持设置涨停不卖，跌停不买，支持设置买卖的涨跌幅区间，当前涨跌幅在区间之外，本次不报单，会根据用户设置的重试次数循环检查符合条件再报单
"""
...
class OrderObject(object):
    ...
    def __init__(self):
        ...
        self.__forbid_sell_when_high_limit = False  # 涨停不卖
        self.__forbid_buy_when_low_limit = False  # 跌停不买
        self.__check_fund_flag = False  # 是否校验资金
        self.__check_position_flag = False  # 是否校验持仓
        self.__buy_low_limit_chg = 0    #买入最低涨跌幅限制
        self.__buy_high_limit_chg = 0   #买入最高涨跌幅限制
        self.__sell_low_limit_chg  = 0  #卖出最低涨跌幅限制
        self.__sell_high_limit_chg = 0  #卖出最高涨跌幅限制
    ...
    def set_loop_check(self, interval_secs=2, try_count=-1):
        """
        设置当资金或持仓不足，或者价格不满足时时，循环检查，满足条件再报单
        :param try_count: 重试次数，默认一直重试
        :param interval_secs:
        :return:
        """
        if interval_secs > 0:
    ...
    def set_check_fund(self):
        """
        设置买入时检查资金是否足够，注意，此时获取到的资金是内存中缓存的资金，与柜台真实资金可能有1.5-2s的延迟
        :return:
        """
        self.__check_fund_flag = True

    def set_check_position(self):
        """
        设置卖出时校验持仓，注意，此时获取的持仓是内存中的持仓，与柜台真实持仓数据可能有延迟
        :return:
        """
        self.__check_position_flag = True

    def set_high_limit_forbidden(self):
        """
        设置涨停不卖，必须订阅tick行情
        :return:
        """
        self.__forbid_sell_when_high_limit = True

    def set_low_limit_forbidden(self):
        """
        设置跌停不买，必须订阅tick行情
        :return:
        """
        self.__forbid_buy_when_low_limit = True

    def set_chg_forbidden(self, buy_low_limit_chg=0, buy_high_limit_chg=0, sell_low_limit_chg=0, sell_high_limit_chg=0):
        """
        设置涨跌幅限制，如果价格超过涨跌幅限制，则禁止报单，0表示无限制
        :param sell_high_limit_chg:卖出最大涨跌幅限制，即超过sell_high_limit_chg则不卖出,传入*100后的值
        :param sell_low_limit_chg:卖出最小涨跌幅限制，即低于sell_low_limit_chg则不卖出,传入*100后的值
        :param buy_high_limit_chg:买入最大涨跌幅限制，即低于buy_high_limit_chg则不买入,传入*100后的值
        :param buy_low_limit_chg:买入最小涨跌幅限制，即低于buy_low_limit_chg则不买入,传入*100后的值
        :return:
        """
        self.__buy_low_limit_chg = buy_low_limit_chg/100.0
        self.__buy_high_limit_chg = buy_high_limit_chg/100.0
        self.__sell_high_limit_chg = sell_high_limit_chg/100.0
        self.__sell_low_limit_chg = sell_low_limit_chg/100.0
    ...
    def set_qty(self, qty):
        ...
        if qty % self.__qty_precision != 0:
            fix_qty = qty - qty % self.__qty_precision
            ...
            if self.high_limit_price < 0.0001:
                ...
                if tick.code != self.symbol:
                    ...
                    if symbol_detial is not None:
                        ...
                        log.info('获取标的{}的涨停价失败，不校验资金，直接报单'.format(self.symbol))
                        return MarketOrderStyle()
                else:
    ...
    def __check_pos(self, qty):
        ...
        :return:
        """
        if not self.__check_position_flag:
            return True
    ...
    def __check_price(self):
        """
        检查价格，如果设置了涨停不卖、跌停不买，则在价格检查的时候会判断
        :return:
        """
        if not self.__forbid_buy_when_low_limit and not self.__forbid_sell_when_high_limit:
            return self.__check_chg()

        if not self.__forbid_sell_when_high_limit and self.side == OrderSide.SELL:
            return self.__check_chg()

        if not self.__forbid_buy_when_low_limit and self.side == OrderSide.BUY:
            return self.__check_chg()

        tick_data = get_current_tick(self.symbol)
        if tick_data.code != self.symbol:
            time.sleep(1)
        tick_data = get_current_tick(self.symbol)
        if tick_data.code != self.symbol:
            log.warn('获取标的[{}]缓存中的实时行情数据失败，本次报单不进行价格检查'.format(self.symbol))
            return True

        if self.__forbid_sell_when_high_limit and tick_data.high_limit - tick_data.current < 0.0001:
            log.info('标的[{}]满足涨停不卖条件，暂时不报单，后续将会循环检查，如果破涨停，将会继续卖出'.format(self.symbol))
            return False
        if self.__forbid_buy_when_low_limit and tick_data.current - tick_data.low_limit < 0.0001:
            log.info('标的[{}]满足跌停不买条件，暂时不报单，后续将会循环检查，如果破跌停，将会继续买入'.format(self.symbol))
            return False
        return self.__check_chg(tick_data)

    def __check_chg(self,tick_data = None):
        """
        校验涨跌幅
        :return:
        """
        if self.side == OrderSide.BUY and self.__buy_high_limit_chg == 0 and self.__buy_low_limit_chg == 0:
            return True
        if self.side == OrderSide.SELL and self.__sell_low_limit_chg == 0 and self.__sell_high_limit_chg ==  0 :
            return True

        if tick_data is None:
            tick_data = get_current_tick(self.symbol)
        if tick_data.code != self.symbol:
            log.warn('获取标的[{}]缓存中的实时行情数据失败，本次报单不进行涨跌幅检查'.format(self.symbol))
            return True
        #计算涨跌幅
        if tick_data.pre_close < 0.0001 or tick_data.current < 0.0001:
            log.warn('标的[{}]缓存中的行情价格异常，本次报单不进行涨跌幅检查，最新价：{}，昨收盘价：{}'.format(self.symbol, tick_data.current, tick_data.pre_close))
            return True

        chg = (tick_data.current - tick_data.pre_close)/tick_data.pre_close
        if self.side == OrderSide.BUY and (chg < self.__buy_low_limit_chg or chg > self.__buy_high_limit_chg):
            log.info('标的[{}]当前涨跌幅[{}]不满足涨跌幅区间限制[{},{}]，行情时间戳：{}，暂时不报单，后续将会循环检查，如果满足条件，将会继续买入'.format(self.symbol, chg,  self.__buy_low_limit_chg, self.__buy_high_limit_chg,tick_data.datetime))
            return False
        if self.side == OrderSide.SELL and (chg < self.__sell_low_limit_chg or chg > self.__sell_high_limit_chg):
            log.info('标的[{}]当前涨跌幅[{}]不满足涨跌幅区间限制[{},{}]，行情时间戳：{}，暂时不报单，后续将会循环检查，如果满足条件，将会继续卖出'.format(self.symbol, chg, self.__sell_low_limit_chg, self.__sell_high_limit_chg,tick_data.datetime))
            return False
        return True
    ...
    def __loop_check_and_reorder(self, context, interval, msg_type):
        ...
        if self.__loop_check_try_count == -1:
            log.debug('重新检查资金、持仓、价格是否满足条件并报单，标的：{}'.format(self.symbol))
            self.__order_exec()

        elif self.__loop_check_try_count - 1 > 0:
            log.debug('重新检查资金、持仓、价格是否满足条件并报单，标的：{}，剩余次数:{}'.format(self.symbol, self.__loop_check_try_count - 1))
            self.__loop_check_try_count = self.__loop_check_try_count - 1
            self.__order_exec()
        else:
            log.debug('检查资金、持仓、价格是否满足条件并报单达到设定的次数，停止检查，标的：{}'.format(self.symbol))
