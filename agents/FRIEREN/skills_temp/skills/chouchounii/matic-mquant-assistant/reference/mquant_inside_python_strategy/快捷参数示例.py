...
def init_trade_files(context, account_type, fund_account, order_file, execution_file, fund_file, pos_file):
    ...
    if not os.path.exists(g.custom_field_file):
        with open(g.custom_field_file, 'w') as f:
            f.write('订单号,自定义字段\n')
    else:
        # 将自定义字段信息都读取出来
        with open(g.custom_field_file, 'r') as f:
    ...
    with open(order_file, 'w') as f:
        ...
        ord_list = get_orders(only_this_inst=False, page_no=1,page_size=-1, account_type=account_type)  # 仅演示获取普通A股账号的订单信息，可根据自己的实际场景选择获取两融的,分页查询，每次获取1000条
        if ord_list is not None and len(ord_list) > 0:
    ...
    with open(execution_file, 'w') as f:
        ...
        execution_list = get_trades(only_this_inst=False,page_no=1,page_size=-1,
...
def record_custom_field(order_id, custom_field):
    ...
    # 将自定义字段写入文件
    try:
        with open(g.custom_field_file, 'a') as f:
...
#    print(ord.order_id, ord.status, ord.fund_account)
    ...
    if len(ord.symbol) == 0 and g.complete_section:
...
def get_execution_str(execution, push_msg=False):
    ...
    '资金账号,成交时间,实例ID,证券代码,买卖方向,成交价格,成交数量,成交金额,成交类型,柜台委托编号,柜台成交编号,自定义字段'
...
#    print('fund_account',fund_account)
    if fund_account != g.fund_account_stock and fund_account != g.fund_account_margin and not g.sub_strategy:
...
#            print(op_type, account_type)
            ...
            if op_type == '0':  # 报单
                ...
                if account_type == AccountType.normal:
                    if entrust_prop == EntrustProp.redemption:  # ETF申赎
                        ...
                        order_req_list.append(item)
                else:
                    entrust_type = params_list[7]
                    amount = qty
                    if side == OrderSide.SELL:
                        amount = -1 * qty
                    contract_no = ''
                    ...
                    if entrust_type == EntrustType.creditTransactions:  # 担保品买卖
                        order_list = MarginTradeHandler.margin_trade(symbol, amount, style)
                        if order_list is not None:
                            record_custom_field(order_list[0].order_id, custom_field)
                            record_order(order_list[0].order_id, symbol, side, qty, price, price_type)
                    elif entrust_type == EntrustType.creditFinancing:  # 卖券还款、融资买入
                        if side == OrderSide.BUY:  # 融资买入
                            order_list = MarginTradeHandler.margincash_open(symbol, qty, style)
                            if order_list is not None:
                                record_custom_field(order_list[0].order_id, custom_field)
                                record_order(order_list[0].order_id, symbol, side, qty, price, price_type)
                        else:  # 卖券还款
                            order_list = MarginTradeHandler.margincash_close(symbol, qty, contract_no, style)
                            if order_list is not None:
                                record_custom_field(order_list[0].order_id, custom_field)
                                record_order(order_list[0].order_id, symbol, side, qty, price, price_type)

                    elif entrust_type == EntrustType.creditMargin:  # 融券卖出、买券还券
                        if side == OrderSide.BUY:  # 买券还券
                            order_list = MarginTradeHandler.marginsec_close(symbol, qty, contract_no, position_type,
                                                                            style)
                            if order_list is not None:
                                record_custom_field(order_list[0].order_id, custom_field)
                                record_order(order_list[0].order_id, symbol, side, qty, price, price_type)
                        else:  # 融券卖出，区分融专项券还是普通券
                            order_list = MarginTradeHandler.marginsec_open(symbol, qty, position_type, style)
                            if order_list is not None:
                                record_custom_field(order_list[0].order_id, custom_field)
                                record_order(order_list[0].order_id, symbol, side, qty, price, price_type)
                    elif entrust_type == EntrustType.returnSecDirect:  # 直接还券
                        order_list = MarginTradeHandler.marginsec_direct_refund(symbol, qty, position_type, contract_no)
                        if order_list is not None:
                            record_custom_field(order_list[0].order_id, custom_field)
                            record_order(order_list[0].order_id, symbol, side, qty, price, price_type)
    ...
    if len(order_req_list) > 0:
        print('报单：', len(order_req_list))
        order_list = orders(order_req_list)
        if order_list is not None:
            for ord, req in zip(order_list, order_req_list):
                # print(ord.__dict__, req.__dict__)
                # 避免多线程的影响，不论是否有自定义字段，都应该记录，收到回报时，检查如果没有自定义字段，则等待
                if ord is not None and hasattr(req, 'custom_field'):
                    record_custom_field(ord.order_id, req.custom_field)
                    
                else:
                    record_custom_field(ord.order_id, '')
                if g.complete_section:
#                    print(req.style)
                    if req.style is not None:
                        price = req.style.get_limited_price()
                        price_type = req.style.get_order_style()
                    else:
                        price = MarketOrderStyle().get_limited_price()
                        price_type = MarketOrderStyle().get_order_style()
                    if req.amount > 0:
                        record_order(ord.order_id, req.security, OrderSide.BUY, req.amount, price, price_type)
                    else:
                        record_order(ord.order_id, req.security, OrderSide.SELL, -1 * req.amount, price, price_type)
