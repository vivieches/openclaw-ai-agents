...
async function resolveForcedFiles(params: {
  ...
  for (const rawIncludePath of params.includeFiles) {
    ...
    try {
      const st = await fs.stat(includePath);
      if (st.isFile()) {
...
export async function scanDirectoryWithSummary(
  ...
  for (const file of files) {
    ...
    // NOTE: Simplified: we don't check allowlist here to avoid internal core dependencies.
    // The Skill Cleaner handles the verification of its own findings afterwards.
    scannedFiles += 1;
